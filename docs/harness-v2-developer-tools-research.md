...
| Approved; queued | Manual splits/resizing and optional cached search preview. `>` commands are implemented in both search fields; the full 1,125-test suite passed. The user accepted these after reviewing primary-source precedents; no permanent clutter or slowing the fast path. Match highlighting is the smaller search refinement. [The six-prototype review](harness-v2-prototype-review.md) records what to borrow and what stays out. |
