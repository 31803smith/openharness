...
| Local Git recovery implemented and regression checked | The older-daemon compatibility reader follows moved worktree metadata, recovers from stopped watchers and expires silently stale context during ordinary discovery. Per-entry refresh is serialized and bounded. No Git subprocess or terminal work is introduced; [performance notes](harness-v2-performance.md) record the limits and evidence. |
...
| Implemented and regression checked | [A coherent agent-workspace key system with file-first remapping](harness-v2-keyboard-system.md). The optional config loads at startup and reloads on save. Workspace and picker dispatch, native menus, search actions and shortcut help share effective bindings; invalid edits keep the last working map. Existing simple defaults remain unchanged. See the progress log for build status and validation. |
| Calibration paused on native focus | A fresh isolated Release fixture built, but macOS reported its visible window inactive and not key. No latency samples were accepted. Resume only after new foreground-access evidence; retain the guard and keep real agents out of timing tests. Live keyboard/tab/drag and real remote interruption checks remain. |
| First-folder path implemented and regression checked | The empty local workspace directly opens the folder chooser, then a prefilled New agent form. Installed-agent discovery overlaps the chooser. Cancellation/stale targets are guarded, and creation remains explicit. Existing agents still open directly; returning users and Cmd-N keep the normal form. [Onboarding notes](harness-v2-appearance-onboarding.md) record recovery, source research and validation limits. |
| Resizing, splits, commands and preview implemented | `>` commands work in both search fields. Controlled resizing uses the existing gaps, saves proportions, protects usable sizes and retains terminal elements. Explicit right/down splits use the normal New agent form and preserve the surrounding rectangles. Cmd-S returns to a preset. Cmd-I or the eye control enables a compact snapshot of already-retained output for the selected agent; preview starts hidden and performs no network or terminal attachment. Both search fields now emphasize matching names and visible context, using the same field preference as ranking and preserving Unicode graphemes; no permanent control is added. The [progress log](harness-v2-progress.md) records validation and the close-layout fallback. [The six-prototype review](harness-v2-prototype-review.md) records what to borrow and what stays out. |
