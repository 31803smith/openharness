...
| Rebuilt and reviewed live | Chrome-style Back/Forward, Recently Closed/Visited and full session History; compact branch-aware headers; Workshop now includes its requested iMac Home agent. |
...
| **In the rebuilt preview** | Chrome-style History; redundant Settings gear removed | Settings stays in the macOS app menu with `Cmd+,`. File groups creation, linking, closing and Reopen Closed Swarm. History exposes Back/Forward, Recently Closed, Recently Visited and Show Full History (`Cmd+Y`). Cmd+P stays in Swarm. Lists are bounded and session-local; stale destinations cannot become unintended new views. The non-macOS toolbar retains Settings access. |
| **In the rebuilt preview** | “Next wallpaper” button and repeated metadata | The permanent wallpaper button is removed; use native View or the New swarm context menu. Headers show one agent title, available branch, machine and controls. Repeated project-name text is removed; full folder/branch context remains on demand. Narrow panes use a machine icon with its full-name tooltip. |
