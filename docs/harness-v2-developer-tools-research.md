...
| Resizing, splits and commands implemented; preview next | `>` commands work in both search fields. Controlled resizing uses the existing gaps, saves proportions, protects usable sizes and retains terminal elements. Explicit right/down splits use the normal New agent form and preserve the surrounding rectangles. Cmd-S returns to a preset. Optional cached search preview and match highlighting remain next. The [progress log](harness-v2-progress.md) records validation and the close-layout fallback. [The six-prototype review](harness-v2-prototype-review.md) records what to borrow and what stays out. |
