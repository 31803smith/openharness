...
| Scope | Result |
| --- | --- |
| Six CLI orchestrator modules | 79 tests; **430/430 lines, 568/568 statements, 313/313 branches, 88/88 functions** |
...
- CLI: real loopback transport, all RPC actions, graph scheduling, immutable artifacts, delivery receipts, concurrent creation, scoped cancellation, restart recovery, storage failures, and fast-worker races.
...
- Native macOS: debug build and a test with three real WKWebViews whose DOMs were read through JavaScript; chat draft/focus survived viewer updates. Native shortcut mapping checks also ran.
