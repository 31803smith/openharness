# Run a local model: one button, one dialog, one agent that starts it
...
- **Pane picker** (`lib/widgets/grid_model_picker.dart`): a last row under Local, always present,
  after the models or after the empty-state sentence: **Run a local model** — drawn as a row, not a
...
- **Native Models menu** (`desktop/macos/Runner/SwarmTitlebar.swift`, `lib/screens/swarm_screen.dart`):
  after the Local section, a separator and an enabled item **Run a local model** dispatching
