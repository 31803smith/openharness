...
Updated 2026-09-13 after unified search, the single-wallpaper decision and the keyboard-system audit. This is a working preview, not a release.

## Current continuation: unified search and keyboard configuration

- The user approved a compact search control at the top right of the existing tab row. It opens the same immediate Cmd-P picker as New swarm's search control. The catalog includes agents, Swarms, machines and projects, fuzzy matching across names/branches/folders/machines, and saved cross-machine project associations. Normalized results are retained across ordinary output notifications; typing makes no file/network requests.
- Return focuses an existing pane (switching Swarms if needed) or opens an unviewed agent in the captured current Swarm. **Add to this swarm**, also Cmd-Enter inside the picker, explicitly shares an existing session here. It reuses the exact pane/session and focuses it for the next terminal key. A stale result or closed target cannot redirect the operation to unrelated work.
- Machine/project results browse their agents inside the picker. Opening the whole reviewed group is an explicit **Open N agents as swarm** action that creates a new Swarm, observes capacity and revalidates member identities. Browsing, filtering and cancellation do not change the current arrangement. New arrivals are not silently added to a reviewed group operation.
...
- Live CUA review confirmed the native search control, immediate search typing, the Workshop project and all three cross-machine agents. It exposed lost field focus after clicking a group; the fix explicitly restores query focus on group entry/back, with a regression check. Repeated project/Swarm names in result subtitles are deduplicated. The final Release preview is running; CUA saw the corrected subtitles. The final native group-entry/Escape retest was interrupted by the user's app activity and remains pending. No more UI actions were sent after reading the fresh state; the user had closed the picker and was in Workshop.
- Verification: **1,074 full Flutter tests passed with one existing skip**, **230 AppKit checks passed including hidden native window layout**, and analyzer reports no errors/warnings (12 existing vendored infos). After the live-review fix, all **20 focused search/history/attention checks passed** and analyzer remained clean apart from the same vendored infos. Both Release builds succeeded and each preview was quit/relaunched normally, one at a time. Logs: `/private/tmp/harness-v2-search-full-tests.log`, `harness-v2-search-titlebar.log`, `harness-v2-search-focus-tests.log`, `harness-v2-unified-search-analyze.log`, `harness-v2-search-focus-build.log`. Native input/display latency measurement remains paused and has no accepted result yet.
...
- The sole final V2 preview was **PID 81588** at the last read-only process check; revalidate before any later restart. Continue on `app-v2`. Do not merge/release, update production daemons, send test input to real agents, restore old layouts or run overlapping V2 previews. The standalone native benchmark remains untracked calibration work; foreground runs are paused while the user is using the app. `docs/prototypes/` is unrelated user work and must be left alone.
