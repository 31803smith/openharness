...
Updated 2026-09-13 after the background-output performance continuation. This is a working preview, not a release.
...
| Full Flutter suite | **1,014 passed, 1 skipped** on the final background-output production code, including terminal, modal, persistence, layout and shortcut regressions. |
...
1. Restore CUA access to the current Release preview and review the new jump workflow, stable canvas, restored terminal output/caret, actual project starters and keyboard navigation. The tool currently reports `cgWindowNotFound`; the process is running. The user can change window size and Swarms; re-read UI state before actions and preserve their arrangement.
