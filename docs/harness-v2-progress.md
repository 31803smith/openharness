...
Updated 2026-09-13 after polishing the editable search bar and making Swarm terminology consistent. This is a working preview, not a release.

## Current continuation: search polish and agent terminology

- **Menu organization is discussion only.** The user asked to study Terminal/iTerm2 and explicitly requested no menu-bar changes yet. Discussed replacing File with Swarm, putting focused-session actions under Agent, retaining Edit/View/Window/Help and keeping Models/Machines/Projects out of separate menus for now. No proposed reorganization is approved or implemented. The agent-removal wording below was already in the preview before this instruction.
...
- Keyboard customization remains the next track. Dispatcher, command catalog, platform validation, inherited configuration and focus-aware host work is preserved; **31 isolated foundation tests pass**. Its incomplete screen/search wiring is parked in `/private/tmp/harness-v2-keyboard-wiring-20260913-search-polish.patch` (SHA-256 `ef2841a963c4392abc612dba186c12ee2c3c05b3cfe87ad06c4ea21d580a6466`) so the preview includes only completed changes. It passes `git apply --check` against this visual update. Native menu/remap integration, command search and dynamic shortcut help still need completion before that wiring is built or shipped. Keep menu edits paused during the user's menu discussion.
