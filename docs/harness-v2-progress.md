...
Updated 2026-09-13 after the pane-focus and disconnected-terminal continuation. This is a working preview, not a release.
...
- Ordinary hidden output now updates the buffer without scheduling a renderer frame. The paired 16/48-terminal debug benchmark reduced hidden-output median CPU cost by 88%/84%; mixed visible/hidden output improved by 32%/51%. Restoring a view reconciles its current buffer, scroll and dimensions once; native caret coordinates coalesce after the frame. That continuation passed 1,014 tests with one skip; the current full suite passes 1,039 with one skip.
- The pane-focus Release preview was rebuilt and normally relaunched; PID **41137** was the sole V2 instance at the last check. Revalidate before any later restart. CUA access returned after this rebuild: the real New swarm, native tab strip and edge-to-edge wallpaper were captured. The user is actively changing window size and Swarms; refresh UI state before actions. Live picker focus verification is still in progress. Earlier `cgWindowNotFound` failures no longer establish a current block. First-frame renderer pixels and native caret messages remain covered by isolated Flutter checks.
...
| Full Flutter suite | **1,039 passed, 1 skipped** on the pane-focus and disconnected-terminal code, including terminal, modal, persistence, layout and shortcut regressions. |
...
1. Continue live CUA review of the jump workflow, stable canvas, restored terminal output/caret, project starters and keyboard navigation. Access returned after the pane-focus rebuild. The user is actively changing window size and Swarms; re-read UI state before actions and preserve their arrangement.
