...
- Native runtime ownership is now implemented behind the still-inactive `keymapState` bridge. Harness menu rows receive effective keys and tooltips; a main-menu dispatcher yields those events to the focused input. The actual native search field uses its own synchronous configured dispatcher, including plain sequence suffixes, with no fallback that revives unbound keys. Standard Edit/Window/font commands retain AppKit ownership. A scoped local monitor is removed on window close.
- **51 native decoder checks and 319 AppKit titlebar checks pass**, including the exported Dart remaps in a hidden native search editor, menu unbinding/rebuilds, sequences, IME, paired key-up, blur and reload. `/private/tmp/harness-v2-keymap-native-runtime.log`. The production Dart file loader/host and effective help are the remaining integration work; the running preview still uses its current defaults.
