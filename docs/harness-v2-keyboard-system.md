...
3. **Native bridge foundation checked:** the Dart adapter exports resolved bindings, contexts, repeat rules, hints and menu-command identities. The native decoder and dispatcher consume that exact exported payload in 51 isolated checks, covering current defaults, context-specific unbinding, remaps, bounded/cancelled sequences, IME, key-up ownership and reload. Logical keys use AppKit's current layout translation; no QWERTY letter table is imposed. The shipped Cmd-I preview action is included in the picker catalog. This foundation is still inactive in the running preview.
4. **Native ownership implemented, awaiting Dart activation:** native menu labels update from the resolved map, including cleared unbindings and complete sequence tooltips. The main menu defers configured first strokes to the focused input. The titlebar search uses a scoped local event monitor so a plain sequence suffix is handled before text insertion; IME, standard text editing, focus changes and window blur retain their appropriate owner. 319 AppKit checks include actual hidden-field remaps and dispatch through the installed monitor. This follows Apple's [key-event routing](https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/EventOverview/HandlingKeyEvents/HandlingKeyEvents.html) and [local event monitor lifecycle](https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/EventOverview/MonitoringEvents/MonitoringEvents.html). Next: connect the Dart file loader and host, remove fixed Flutter fallbacks, and update help/tooltips from the same bindings before enabling customization.
5. **Validation:** exact action and focus destinations, custom remaps, unbinding, repeated movement, sequences, cancellation, modal/IME behavior, and live config reload in isolated agent fixtures. Do not type tests into real agents.
6. **Delivery:** rebuild and inspect the preview only after integration passes. The broader [working queue](harness-v2-developer-tools-research.md) tracks product proposals separately from authorized fixes.
