...
4. **Next implementation:** connect runtime configuration to the workspace and search, remove competing native shortcut owners when Flutter owns input, and update menu/help/tooltips from the same bindings. Preserve the agreed defaults. A shortcut's native menu label must not cause the old binding to survive unbinding in Flutter.
