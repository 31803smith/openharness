...
class PaneLayoutStore {
  ...
  /// next launch compares what it remembers with what the daemon now serves
  /// and re-keys then. Without this, every tile waits for a machine that no
  ...
  ///
  /// The one case: this computer changes id when the account under it does.
  /// Signed out the daemon serves it under the computer id, signed in under
  /// the account's machineId, and a sign-in or sign-out swaps the two. The
  /// tiles are intent about THIS computer either way; the id is a fact about
  /// the daemon. Rewriting the file and restoring from it is how the desk
  /// follows without a second, live way of re-keying a tile.
  ///
  ...
  Future<void> rekeyMachine({
    ...
    for (final raw in saved['swarms'] as List) {
      ...
      // Focus and zoom are indexes into this list; a shorter list is repaired
      // by the restore, which clamps them to the first tile.
      raw['panes'] = kept;
    }
    if (!changed) return;
    try {
      ...
      // The next launch restores the old ids; a tile that then waits for a
      // machine is the smaller wrong than a sign-in that throws.
    }
  }
