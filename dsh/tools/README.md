...
```sh
node dsh/tools/dsh-e2e.mjs <machineId> <dshId> <engine> <workspace> [--keep] [--restart] [--bypass] [--no-verdict]
    # create → materialize → viewer up → HARNESS_DSH on the pane → (verdict) → delete
node dsh/tools/dsh-discover.mjs ...   # a pane started by hand with HARNESS_DSH is discovered as its harness
node dsh/tools/dsh-delete.mjs <agentId> <machineId>
```
