...
## a board attached to check the result. See device/harness-device/firmware/RELEASE.md.
##
...
upload-circle:
	bash device/harness-device/firmware/scripts/upload-firmware.sh $(ARGS)
...
device-test:
	bash device/harness-device/firmware/test/run.sh
