...
`make upload-circle` just runs `device/esp32-circle/scripts/upload-firmware.sh` (a full pipeline: bump
`version.txt` → `idf.py -C device/esp32-circle build` → upload `.bin` + merge `metadata.json` to GCS → **git
...
- The version baked **into the binary** (`esp_app_desc.version`) and the version in the **manifest**
  both come from the same `device/esp32-circle/version.txt`, so the device's equality check is exact. Keep them
  in sync by only ever changing the version there.
- If a rebuild doesn't seem to pick up the new version, `touch device/esp32-circle/version.txt` before step 3
