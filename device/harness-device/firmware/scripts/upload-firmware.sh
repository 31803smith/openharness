...
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"    # device/harness-device/firmware
...
if [ "$DO_BUILD" -eq 1 ]; then
  ...
  # tracked defaults alone. The developer's interactive device/harness-device/firmware/sdkconfig is left untouched.
