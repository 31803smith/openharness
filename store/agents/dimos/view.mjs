...
export function mount(stage,api){
  ...
  const screen=canvas(stage,(ctx,w,h,t)=>{
    now=t;const size=Math.min(h-85,w-70),left=(w-size)/2,top=22,s=size/10;bounds={left,top,s};const map=(x,y)=>[left+x*s,top+(10-y)*s];
    ...
    words(ctx,'1 METRE GRID',left,top+size+20,'#8da684',9);
