...
SCRIPTS = ("fetch-upstream.sh", "setup.sh", "doctor.sh", "init-workspace.sh", "viewer.sh")
...
STUB = '#!/bin/sh\necho "{name} dsh=$HARNESS_DSH_DIR toolchain=${{CIRCUIT_TOOLCHAIN:-}} pwd=$(pwd -P)"\nexit "${{STUB_EXIT:-0}}"\n'
...
class Wrapper(unittest.TestCase):
    ...
    def test_doctor_at_the_pin_hands_over_to_the_projects_doctor(self):
        ...
        self.assertEqual(self.real(Path(re.search(r"toolchain=(\S+)", project).group(1))), self.real(self.pkg / "upstream" / "toolchain"))
    ...
    def test_doctor_keeps_a_toolchain_the_environment_names(self):
        self.fake_fetched(self.upstream.first)
        run = self.run_script("doctor.sh", CIRCUIT_TOOLCHAIN="/Users/example/circuit/toolchain")
        self.assertIn("toolchain=/Users/example/circuit/toolchain ", run.stdout)
    ...
    def test_init_and_viewer_run_the_projects_own_in_the_workspace(self):
        self.fake_fetched(self.upstream.first)
        ws = self.tmp / "workspace"
        ws.mkdir()
        for script, word in (("init-workspace.sh", "init-workspace"), ("viewer.sh", "viewer")):
            run = self.run_script(script, cwd=ws)
            self.assertEqual(run.returncode, 0, run.stderr)
            self.assertTrue(run.stdout.startswith(f"{word} "), run.stdout)
            ...
            self.assertEqual(self.run_script(script, cwd=ws, STUB_EXIT="5").returncode, 5)
