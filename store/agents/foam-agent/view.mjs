...
export function mount(stage,api){
  let p=api.getParameters(),field,pressure=false,arrows=false,tracers=[],lastT=0;
  const info=label(stage,'An invitation to follow the flow.'),tag=badge(stage,'FLOW SKETCH');
  const screen=canvas(stage,(ctx,w,h,t)=>{
    grid(ctx,w,h,32);const top=38,bottom=h-55,fh=bottom-top,scaleX=(w-48)/120,scaleY=fh/56;
    const speed=p.speed/100,cx=24+36*scaleX,cy=top+28*scaleY;
    if(field){
      for(let row=0;row<field.ux.length;row++)for(let col=0;col<field.ux[row].length;col++){
        ...
        ctx.fillStyle=`hsla(${205-v*150},60%,${25+v*30}%,${.06+v*.3})`;ctx.fillRect(24+col*2*scaleX,top+row*2*scaleY,2*scaleX+1,2*scaleY+1);
        if(arrows&&col%4===0&&row%4===0){const x=24+col*2*scaleX,y=top+row*2*scaleY;line(ctx,[[x,y],[x+field.ux[row][col]*90,y+field.uy[row][col]*90]],'#d0eeec66',1);}
      }
    }
    for(let row=0;row<19;row++){
      const y0=top+row/18*fh,points=[];
      for(let x=24;x<w-24;x+=4){const near=Math.exp(-(((x-cx)/(30*scaleX))**2)),dy=(y0-cy),spread=Math.sign(dy)*p.radius*scaleY*Math.exp(-((dy/(p.radius*scaleY*2))**2))*near;points.push([x,y0+spread]);}
      line(ctx,points,field?'#b3e5e41b':'#b3e5e42f',.8);
      if(!field){const x=24+((t*p.speed*22+row*53)%(w-48)),near=Math.exp(-(((x-cx)/(30*scaleX))**2)),dy=y0-cy;circle(ctx,x,y0+Math.sign(dy)*p.radius*scaleY*Math.exp(-((dy/(p.radius*scaleY*2))**2))*near,1.4,'#a1dadba0');}
    }
