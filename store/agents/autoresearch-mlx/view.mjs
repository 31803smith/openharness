...
export function mount(stage,api){
  ...
  const screen=canvas(stage,(ctx,w,h)=>{
    const left=58,right=w-28,top=52,bottom=h-80,curve=result?.data?.curve;
    ...
    words(ctx,'BITS / CHARACTER',18,26,'#88a7ab',9);for(let i=0;i<5;i++)words(ctx,String(Math.round(maxStep*i/4)),x(maxStep*i/4)-5,bottom+23,'#88a7ab',10);
