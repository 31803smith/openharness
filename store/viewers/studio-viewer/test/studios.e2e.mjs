...
const domains={
  ...
  'simskill':async page=>{await page.getByRole('button',{name:'Pause traffic',exact:true}).click();await page.getByRole('button',{name:'Play traffic',exact:true}).click();await page.getByRole('button',{name:'8× speed',exact:true}).click();await page.getByRole('button',{name:'16× speed',exact:true}).click();await page.getByRole('button',{name:'1× speed',exact:true}).click();await page.getByRole('button',{name:'Follow a car',exact:true}).click();await page.getByRole('button',{name:'See the city',exact:true}).click();await page.getByRole('slider',{name:'Traffic replay position'}).focus();await page.keyboard.press('ArrowRight');},
  'bonsai-mcp':async page=>{await page.getByRole('button',{name:'Floor plan',exact:true}).click();await page.getByRole('button',{name:'Next room',exact:true}).click();await page.getByRole('button',{name:'Orbit view',exact:true}).click();await page.getByRole('button',{name:'Separate floors',exact:true}).click();await page.getByRole('button',{name:'Bring together',exact:true}).click();const box=await page.locator('canvas').boundingBox();await page.mouse.move(box.x+box.width/2,box.y+100);await page.mouse.down();await page.mouse.move(box.x+box.width/2+70,box.y+100,{steps:5});await page.mouse.up();await page.locator('canvas').focus();await page.keyboard.press('ArrowRight');},
  ...
  for(const name of names){
    ...
    try{
      ...
      for(const c of config.controls.filter(c=>!c.hidden)){
        ...
        else if(c.type==='select'){await input.selectOption(c.options.at(-1).value);await input.selectOption(c.value);}
      ...
      await page.locator('#remote').waitFor({state:'visible'});await page.getByRole('button',{name:'Load update',exact:true}).click();
      ...
      const coverage=await page.coverage.stopJSCoverage();await writeFile(join(output,`${name}-browser-coverage.json`),JSON.stringify(coverage));
      report.push({name,status:'passed',artifactCount:current.result.artifacts.length,engine:current.result.engine,workflows:['doctor','initialization','viewer','all controls','domain interactions','run','artifacts','download','history','pause','agent edit','reload','narrow layout','reduced motion','malformed project recovery']});
