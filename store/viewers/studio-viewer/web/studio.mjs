...
function controls(config) {
  ...
  for(const c of config.controls){
    ...
    input.name=c.id;input.id=`control-${c.id}`;input.value=parameters[c.id];label.htmlFor=input.id;
    const update=()=>{output.value=c.type==='number'?`${input.value}${c.unit?' '+c.unit:''}`:'';};update();input.addEventListener('studio-value',update);
...
function renderResult(result){
  ...
  for(const a of result?.artifacts??[]){const link=text('a',`↓  ${a.label}`,'artifact');link.href=artifactURL(a.path)+'?download';link.download='';$('artifacts').append(link);}
...
async function refresh(){
  try{
    ...
    else if(state.job?.status==='cancelled')notice(state.job.message);
